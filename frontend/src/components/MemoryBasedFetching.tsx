...
import MemoryBasedSuggestion from './MemoryBasedSuggestion';
...
const ImageGallery = () => {
  ...
  const handleCollectLandmarks = () => {
    const allLandmarks = images.flatMap(image => 
      image.landmarks.map(landmark => landmark.name)
    ).join(", "); // Join the array elements with a comma and a space
    setVisitedPlaces(allLandmarks);
    setShowMemorySuggestion(true); // Show only the MemoryBasedSuggestion component
  };
  ...
  // If showMemorySuggestion is true, render only the MemoryBasedSuggestion component
  if (showMemorySuggestion) {
    return <MemoryBasedSuggestion initialVisitedPlaces={visitedPlaces} triggred={true} />;
  }

  // Otherwise, render the ImageGallery content
  return (
